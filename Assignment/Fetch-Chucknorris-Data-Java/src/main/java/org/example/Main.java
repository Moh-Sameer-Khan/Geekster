...
//        bcz we are using anywhere on other places but these properties was inside try catch so we use like this -- > who we can access anywhere
        URL url = null;  // error coming so we pass null
        HttpURLConnection connection = null; // error coming so we pass null
        int responseCode = 0; // // error coming so we pass 0
...
//            Step -- 5.2 --> we want to string Form -- StringBuilder -- use bcz mutable can add data
            StringBuilder apiData = new StringBuilder();

//            Step -- 5.2 --> For reading
...
//      Step -- 6   --> Clode the connection -- here close but good use in finally block ,close --> its thrwos Exception
            try {
                bufferIn.close();
            } catch (IOException e) {
